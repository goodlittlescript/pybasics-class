...
#
# The return part in a list comprehension can "do things"
#

price_changes = [m["close"] - m["open"] for m in measurements]
print(f"Max price change: {max(price_changes)}")
...
price_changes_in_may = [
    m["close"] - m["open"] for m in measurements if m["date"].startswith("05")
]
print(f"Max price change in May: {max(price_changes_in_may)}")
print(f"Min price change in May: {min(price_changes_in_may)}")

#
# In other words they are a shorthand for a loop:
#

price_changes_in_may = []
for m in measurements:
    if m["date"].startswith("05"):
        price_change = m["close"] - m["open"]
        price_changes_in_may.append(price_change)
